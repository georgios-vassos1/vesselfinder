...
def test_store_enrichment_executes_upsert_per_entry():
    mock_conn, mock_cur = _mock_conn()
    store_enrichment(mock_conn, {"2b4e1f": "400EA1", "3c4563": "3C6445"})
    assert mock_cur.execute.call_count == 2
    mock_conn.commit.assert_called_once()


def test_store_enrichment_row_contains_flight_id_and_icao24():
    mock_conn, mock_cur = _mock_conn()
    store_enrichment(mock_conn, {"2b4e1f": "400EA1"})
    args = mock_cur.execute.call_args[0][1]
    assert args[0] == "2b4e1f"
    assert args[1] == "400EA1"
